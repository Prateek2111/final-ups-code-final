...
import { primaryApi, fallbackApi, safeApiCall } from './services/apiService';
...
export default function App() {
  ...
  const fetchLoads = useCallback(async () => {
    ...
    try {
      const loads = await safeApiCall((api) => api.fetchLoads());
      if (isToggling.current) return;
  ...
  const fetchFromBackend = useCallback(async () => {
    try {
      const [sensor, settings] = await Promise.all([
        safeApiCall((api) => api.fetchSensorData()),
        safeApiCall((api) => api.fetchSettings()),
      ]);
      ...
      setSystemDcCurrent(sensor.dcCurrent || 0.0);
  ...
  const handleSetRelayState = async (id, targetState = null) => {
    ...
    try {
      const state = await safeApiCall((api) => api.setLoadState(id, desired));
  ...
  // Simulation handlers
  const handleSimEnvUpdate = async (
    temp = systemTemp,
    hum = systemHum,
    dist = sysDist,
    batt = systemBattery,
    ...
    try {
      await primaryApi.updateEnvironment(payload);
    } catch {
      try {
        await fallbackApi.updateEnvironment(payload);
      } catch {
  ...
  const handleSimulatePowerFailure = () => {
    ...
    handleSimEnvUpdate(systemTemp, systemHum, sysDist, systemBattery, 0.0);
    applyAutomationLogic();
  };
  ...
  const handleRestoreUtility = () => {
    ...
    handleSimEnvUpdate(systemTemp, systemHum, sysDist, systemBattery, 220.0);
    applyAutomationLogic();
  };
  ...
  const handleDrainBattery = () => {
    ...
    handleSimEnvUpdate(systemTemp, systemHum, sysDist, next, systemVoltage);
    applyAutomationLogic();
  };
  ...
  const handleTempUp = () => {
    ...
    setSystemTemp(next);
    handleSimEnvUpdate(next, systemHum, sysDist, systemBattery, systemVoltage);
  };
  ...
  const handleHumidityUp = () => {
    ...
    setSystemHum(next);
    handleSimEnvUpdate(systemTemp, next, sysDist, systemBattery, systemVoltage);
  };
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto p-4 sm:p-6 pb-24 sm:pb-28">
        {tabIndex === 0 && (
          <OverviewTab
            ...
            systemDcVoltage={systemDcVoltage}
            systemDcCurrent={systemDcCurrent}
